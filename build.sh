...
# Exactly two standard artifacts, chosen by BUILD_TYPE alone:
#   Production (default):  ./build.sh pico_w
#   Debug:                 ./build.sh pico_w debug
# CMakeLists.txt derives SIDETNFS_ENABLE_DEBUG/SIDETNFS_ENABLE_DIAG from
# this same DEBUG_MODE, so no other flag is needed for either variant --
# see that file for the (still independently overridable, for advanced
# testing) env vars behind them.
if [ "$BUILD_TYPE" = "release" ]; then
...
# Copy the built firmware to build_artifacts/ under its variant name --
# exactly one file per invocation: sidetnfs_production.uf2 or
# sidetnfs_debug.uf2.
