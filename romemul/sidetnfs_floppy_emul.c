 ...
 *   1. Open image_path (a COMPLETE path -- TNFS always mounts "/", so
 *      there is no per-source mount path any more) on the caller-supplied
 *      source (mixed-source redesign -- no profile_index/flash lookup;
 *      mirrors sidetnfs_floppy_browse_open()'s own backend resolution,
 *      adapted for opening one file instead of a directory).
...
// "0:" + image_path -- FatFS's own drive-number prefix. Mixed-source
// redesign: image_path is now always a COMPLETE SD path (no more
// per-source sd_path root to join against), so this is a plain prefix,
// not a join.
static bool prefix_sd_drive(const char *image_path, char *out, size_t out_size)
{
    int n = snprintf(out, out_size, "0:%s", image_path);
    return n > 0 && (size_t)n < out_size;
}

// Resolves `source`, opens image_path (a COMPLETE path from the TNFS
// server's own root -- it always mounts "/" -- or a complete SD path)
// into *st, and validates it (file size, BPB, geometry). Shared by
// sidetnfs_floppy_emul_open() (targets s_state) and
// sidetnfs_floppy_emul_open_candidate() (targets s_candidate) -- see the
// header's own comment for why those two need separate targets. Body
// defined further down (after close_state()/read_sector_raw_from(), which
// it uses) -- forward-declared here so sidetnfs_floppy_emul_open() can
// stay at its original, early position in this file.
static sidetnfs_floppy_emul_status_t open_into(floppy_emul_state_t *st, const sidetnfs_floppy_source_t *source,
                                                 const char *image_path, bool network_ok,
                                                          ...
                                                          bool network_ok, sidetnfs_floppy_geometry_t *out_geom)
{
    ...
    sidetnfs_floppy_emul_close();
    return open_into(&s_state, source, image_path, network_ok, out_geom);
}
...
// Full body of the open_into() forward-declared near the top of this
// file -- see that declaration's own comment. Resolves `source` (backend
// + TNFS host/port, mixed-source redesign -- no more profile_index/flash
// lookup), opens image_path (a COMPLETE path; TNFS always mounts "/" so
// there is no separate mount_path any more) into *st, and validates it
// (file size, BPB, geometry -- see this file's own top-of-file comment
// for the exact algorithm). Every failure path closes *st before
// returning, so a caller never has to remember to clean up itself.
static sidetnfs_floppy_emul_status_t open_into(floppy_emul_state_t *st, const sidetnfs_floppy_source_t *source,
                                                 const char *image_path, bool network_ok,
                                                 sidetnfs_floppy_geometry_t *out_geom)
{
    ...
    if (source == NULL || image_path == NULL || image_path[0] == '\0')
    {
        return SIDETNFS_FLOPPY_EMUL_ERR_INVALID_SOURCE;
    }
    ...
    if (source->backend == SIDETNFS_FLOPPY_SOURCE_TNFS)
    {
        ...
        // one active browse/mount at a time. Always mounts "/" now (the
        // mixed-source redesign's fixed convention). Re-populates only
        // when host/port actually differ from whatever this shared slot
        // currently holds, so switching back to a previously-used TNFS
        // server (e.g. a Carousel returning to an earlier entry) doesn't
        // always pay a fresh MOUNT round trip.
        ...
        st->backend = FLOPPY_EMUL_BACKEND_TNFS;
        st->tnfs_slot = slot;
        st->tnfs_handle = handle;
        st->handle_open = true;
    ...
    else if (source->backend == SIDETNFS_FLOPPY_SOURCE_SD)
    {
        ...
        char full_path[SIDETNFS_FLOPPY_FAVORITE_PATH_MAX + 4];
        if (!prefix_sd_drive(image_path, full_path, sizeof(full_path)))
        {
            return SIDETNFS_FLOPPY_EMUL_ERR_PATH_TOO_LONG;
        }

        FRESULT fr = f_open(&st->sd_file, full_path, FA_READ);
        if (fr != FR_OK)
        {
        ...
        st->backend = FLOPPY_EMUL_BACKEND_SD;
        st->handle_open = true;
        filesize = (uint32_t)f_size(&st->sd_file);
    }
    else
    {
        return SIDETNFS_FLOPPY_EMUL_ERR_INVALID_SOURCE;
    ...
    if (read_sector_raw_from(st, 0, sector0) != SIDETNFS_FLOPPY_EMUL_OK)
    {
        close_state(st);
        return SIDETNFS_FLOPPY_EMUL_ERR_READ_FAILED;
    }
...
// applied to the candidate slot instead of the active one.
                                                                     ...
                                                                     sidetnfs_floppy_geometry_t *out_geom)
{
    ...
    return open_into(&s_candidate, source, image_path, network_ok, out_geom);
}
