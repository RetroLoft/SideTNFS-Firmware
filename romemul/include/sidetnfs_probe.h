...
// with an explicit backend identifier, so future backends (e.g.
...
// Compatibility macros -- gemdrvemul.c/sidetnfs_probe.c's main
...
// SIDETNFS_EXPERIMENTAL_FS_LISTING (the original on/off
...
// The RAM directory-cache layer (root pre-cache, cache slots,
...
// didn't fit the GEMDOS/DTA/Fsnext flow cleanly (
// SIDETNFS_PHASE5_DIRECTORY_LISTING.md). The TNFS DTA-registry model
// (sidetnfs_tnfs_dta_start/next) is now unconditionally the
...
// How many entries GEMDRVEMUL_FSFIRST_CALL/FSNEXT_CALL's TNFS DTA
...
// Number of concurrent TNFS directory searches (one per active
// ndta) the TNFS DTA-registry can track -- the direct TNFS-side analogue of
// the existing FatFS insertDTA()/lookupDTA() hash table (
...
// SIDETNFS_LIVE_SEARCH_SLOTS  -- same mechanism, explicitly framed
// as a DTA registry instead of a bespoke "live search" concept.
#ifndef SIDETNFS_TNFS_DTA_SLOTS
#define SIDETNFS_TNFS_DTA_SLOTS 8
#endif

// Hard cap on READDIRX round-trips per single Fsfirst/Fsnext call
...
// ever writes DEBUG.TXT; only the SELECT edge-handler does. this
...
// SIDETNFS_ENABLE_DEBUG by default, the single knob the
...
typedef struct
{
    ...
    // --- per-slot MOUNT session administration  ---
    ...
    // True once this slot's own MOUNT
...
// No longer special-cases slot 0/1 -- every valid TNFS slot's
...
// Send a single UDP packet to the TNFS server. Kept for reference;
// not called automatically. Must only be called after WiFi is
...
// Check whether the MOUNT response is in and successful, and if so
...
// If the debug state is dirty (mount probe just sent, or a
...
// Mark that networking/TNFS was skipped this boot (no WiFi
...
// Introduced sidetnfs_note_tnfs_io/sidetnfs_note_sd_io and four
// I/O counters here as one-time backend-isolation review instrumentation.
// Removed them: the isolation review is complete, its findings are
...
// One-shot SD/FatFS root directory scan (via f_opendir/f_readdir),
...
// A TNFS OPENDIRX/READDIRX entry normalized into Atari/GEMDOS form.
...
typedef struct
{
    ...
    uint16_t date;  // DOS/GEMDOS date -- fixed placeholder for now
    uint16_t time;  // DOS/GEMDOS time -- fixed placeholder for now
...
// Strict, uppercase-only 8.3 name check. Rejects anything that
...
// Convert one TNFS OPENDIRX/READDIRX entry into Atari/GEMDOS form.
...
// Local GEMDOS-style wildcard match against an already-normalized
...
// sides are expected to already be uppercase 8.3. No malloc,
// no I/O.
bool sidetnfs_gemdos_pattern_match(const char *name83, const char *pattern);

// Match one normalized entry's Atari/GEMDOS attribute byte (see
...
typedef enum
{
    ...
    SIDETNFS_DIR_SEARCH_ERROR,       // cache-miss refresh could not be resolved in time -- no SD fallback
} SidetnfsDirSearchResult;

// True if a TNFS-backed Fsfirst/Fsnext may be attempted this boot:
// MOUNT succeeded, and the existing OPENDIRX/READDIRX root probe (
...
// Start a fake, memory-only directory search for ndta -- used
...
// (which may already be torn down in this case. path == "/"
// yields exactly one synthetic entry, "NO_NETW.TXT" (a plain file, size 0,
// date/time 0/0 -- opening it is not expected to work yet; any
...
// Per-runtime-drive error status, for
...
// Exact filename shown for a not-ready ENABLED TNFS drive's
...
// -- the two must never disagree. Pure formatting,
...
// Sibling of sidetnfs_fake_search_start above, for an ENABLED
...
// Start a TNFS directory search for ndta -- OPENDIRX for path,
...
// f_findfirst()+insertDTA() contract (
...
// repeated Fsfirst
// the repeat-continuation workaround was removed.
...
// Look up ndta's registry entry (lookupTnfsDTA) and continue the
...
// No slot parameter -- reads the slot
...
// Release ndta's TNFS DTA-registry entry (releaseTnfsDTA), if
...
// the server/session to reclaim; this only clears local
// bookkeeping so the slot can be reused.
void sidetnfs_tnfs_dta_release(uint32_t ndta);

// Number of currently-active TNFS DTA-registry entries -- used by
...
// TNFS file-op results. Mirrors the FOUND/NOT_FOUND/ERROR shape of
...
// , use sidetnfs_tnfs_file_create below for Fcreate instead.
...
//
// Runtime_slot selects whose TNFS
...
// Open tnfs_path over TNFS for GEMDOS Fcreate -- always
...
//
// Runtime_slot -- read from
// the caller's own FileDescriptors entry (set at Fopen/Fcreate time,
// ) -- selects whose TNFS session/host/port this READ resolves via
...
// Runtime_slot -- same meaning as sidetnfs_tnfs_file_read above.
...
// back to the caller --, the local file descriptor
...
// One-shot TNFS directory-existence probe for GEMDRVEMUL_DSETPATH_CALL
...
// TNFS SEEK for an already-open file handle. seek_from_end==false
...
// (nullable) receives the raw TNFS wire rc byte (0xFF if
...
// TNFS delete results. Mirrors the SidetnfsFileOpenResult shape
...
// TNFS UNLINK for a file (never a directory -- see
...
// mapping itself).
// Runtime_slot -- the ROM-resolved slot the caller's own
...
// TNFS rename results. Same shape/reasoning as
...
// TNFS RENAME from old_path to new_path (both already-resolved
...
// Runtime_slot -- both old_path and new_path must already have
...
// TNFS mkdir results. PATH_NOT_FOUND is unambiguous here (unlike
...
// TNFS MKDIR for tnfs_path (already-resolved TNFS-relative path).
...
// Runtime_slot -- same meaning as sidetnfs_tnfs_file_delete
...
// TNFS rmdir results. PATH_NOT_FOUND covers both "directory
...
// TNFS RMDIR for tnfs_path (already-resolved TNFS-relative path).
...
// TNFS attribute-query/set results for GEMDRVEMUL_FATTRIB_CALL.
...
// TNFS STAT -> GEMDOS/DOS-style (FS_ST_*) attributes, for Fattrib
...
// Runtime_slot -- shares tnfs_stat_raw with
// sidetnfs_tnfs_get_datetime() below.
// GEMDRVEMUL_FATTRIB_CALL's own call site now passes the
...
// flag/new-attribute value) -- the prefix-only resolution and
...
// For Fattrib set (wflag 1). Confirmed against the actual server
...
// Runtime_slot -- read from the caller's FileDescriptors entry
...
// For Fdatime set (wflag 1). The actual server's protocol header
...
// Release any active TNFS DTA-registry search (see
...
// No longer called by Ddelete (a still-open OPENDIRX
...
// Close (CLOSEDIR) every active TNFS DTA-registry
...
// Runtime_slot -- a registration now matches only when BOTH its
...
// Continue the active fake no-network search for ndta. Pure
...
// True if ndta has a currently active fake no-network search.
bool sidetnfs_fake_search_is_active(uint32_t ndta);

// Explicitly close the active fake no-network search for ndta, if
...
// Number of currently-active fake no-network searches -- see
// sidetnfs_tnfs_dta_count_active() above.
uint16_t sidetnfs_fake_search_count_active(void);

// Record one successful/failed TNFS Fsfirst/Fsnext hit for the
...
// Record one GEMDRVEMUL_FSEEK_CALL outcome for the
...
// Record one GEMDRVEMUL_FDELETE_CALL outcome for the DEBUG.TXT
...
// Record one GEMDRVEMUL_FRENAME_CALL outcome for the DEBUG.TXT
...
// Record one GEMDRVEMUL_DCREATE_CALL outcome for the DEBUG.TXT
...
// Record one GEMDRVEMUL_DDELETE_CALL outcome for the DEBUG.TXT
...
// Record one sidetnfs_tnfs_dta_close_by_path outcome
...
// -diag: record one GEMDRVEMUL_DDELETE_CALL outcome
...
// Record one GEMDRVEMUL_DDELETE_CALL cwd-target-match/
...
// Record one GEMDRVEMUL_WRITE_BUFF_CALL outcome for the DEBUG.TXT
...
// Record one GEMDRVEMUL_FATTRIB_CALL outcome for the DEBUG.TXT
...
// Record one GEMDRVEMUL_FDATETIME_CALL outcome for the DEBUG.TXT
...
// RAM-only Fsfirst/Fsnext diagnostic eventlog. See
...
typedef enum
{
    ...
    // SIDETNFS_DIAG_SEARCH_OVERWRITE still used (fake no-network
    ...
    // Repeated-Fsfirst-as-continuation diagnostics REMOVED --
    // superseded by the real fix.
    SIDETNFS_DIAG_FSNEXT_CASE_REACHED,
    // Every dispatched command_id (
    ...
    // TNFS OPENDIRX/READDIRX events (renamed from
    ...
    // SD-baseline measurement events. Logged from the SD/FatFS
    ...
    // behavior is touched. DTA_LOOKUP_OK/FAIL and DTA_INSERT are
    ...
    // TNFS DTA-registry events -- the TNFS-side analogue of
    ...
    // GEMDRVEMUL_DTA_EXIST_CALL/GEMDRVEMUL_DTA_RELEASE_CALL events
    ...
    // issue a real Fsnext at all (
 // COMMAND_ENTER DTA_EXIST immediately before FSNEXT_ENTER). Before
 ...
 // TNFS CLOSEDIR events -- always sent (see tnfs_dta_closedir
    ...
    // A mutating GEMDOS trap was denied at SIDETNFS_BACKEND_TNFS
    // (
    ...
    // Real TNFS-backed Fopen(mode 0 only)/Fread/Fclose. ENTER/
    ...
    // Exact-faaltrap diagnosis. GEMDRVEMUL_DSETPATH_CALL is
    ...
    // higher-level events above -- purely additive, nothing above
    ...
    // Local file->offset before/after a READ_BUFF_CALL, logged
    ...
    // Dsetpath/Dgetpath/current-directory backend-aware. The
    ...
    // TNFS Fseek. ENTER/HANDLE/MODE/OFFSET_IN/OFFSET_OUT/RETURN
    ...
    // TNFS Fdelete (files only -- Ddelete/directories untouched).
    ...
    // is no longer reachable for the file-delete path but is
    // left defined.
    ...
    // TNFS Frename (files, and directories when the server
    ...
    // TNFS Dcreate. ENTER/RAW_PATH/TNFS_PATH/RETURN logged from
    ...
    // TNFS Ddelete (single empty directory only -- never
 ...
 // SIDETNFS_DIAG_DDELETE_DTA_RELEASE is superseded by the
    ...
    // left defined, no longer emitted.
    ...
    // Pre-RMDIR targeted directory-search-handle close.
    ...
    // The local target-path == current-working-directory
    ...
    // TNFS Fwrite -- deliberately minimal (
    // ACK handshake is timing
    ...
    // 7Lb: TNFS Fattrib -- deliberately minimal, same reasoning as
    // 's Fwrite (compact counters, only failure/decision events, no
    // per-field detail). STAT_ERROR covers inquire (set no longer sends
    // STAT eitherb). SET_UNSUPPORTED is now the *only* set
 ...
 // an empty function body, so removed the CHMOD call entirely
    ...
    // TNFS Fdatime -- same deliberately-minimal reasoning as
    // 's Fattrib. INQUIRE_OK/INQUIRE_ERR logged from
    ...
    // GEMDOS Dfree under TNFS -- tnfsd 24.0522.1 has no free/total
...
// Record one diagnostic event. Pass
...
typedef struct
{
    ...
    // the trace comes from the ROM source itself.
